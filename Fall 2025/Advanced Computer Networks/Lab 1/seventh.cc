...
main (int argc, char *argv[])
{
  ...
  pointToPoint.SetChannelAttribute ("Delay", StringValue ("5s")); // dynamic link delay

  NetDeviceContainer devices;
  devices = pointToPoint.Install (nodes);

  Ptr<RateErrorModel> em = CreateObject<RateErrorModel> ();
  em->SetAttribute ("ErrorRate", DoubleValue (0.0002)); // dynamic error rate
  ...
  Simulator::Stop (Seconds (20)); // simulation timeout in seconds
  Simulator::Run ();
  Simulator::Destroy ();
