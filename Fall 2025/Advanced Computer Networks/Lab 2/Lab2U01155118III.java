...
public class Lab2U01155118III {
    ...
    static Clock clock = new Clock(); // our one and only "clock" instance ;)
    ...
    static class Event {
         ...
         * and i would have to instantiate event to `eligibleDepartures` and
    ...
    // advance (change/set) virtual time and real time using backlog weight.
    static void advanceTo(double newRealTime, double weight) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean wf2qIsEnabled = Arrays.asList(args).contains("--enable-wf2q");
        // reader and writer
        ...
        while (numberOfDepartedPackets < numberOfPackets) {
             ...
             * advance to that arrival time first.
             */
            while (!arrivalEvents.isEmpty() &&
                ...
                // advance virtual time to the exact arrival instant AND must compute weight.
                ...
                // advance to arrival instant
                advanceTo(arrivedPacket.arrivalTime, totalBackloggedWeight);
            ...
            if (packetInProgress == null) {
                ...
                // if still no eligible packets, we need to advance time.
                if (eligibleDepartures.isEmpty()) {
                    ...
                    if (nextArrivalTime < clock.realTime + boundaryDelta - EPS) {
                        ...
                        advanceTo(nextArrivalTime, totalBackloggedWeight);
            ...
            if (packetInProgress == null && !eligibleDepartures.isEmpty()) {
                if (wf2qIsEnabled) {
                    // our choosen packet of course
                    Packet selectedPacket = null;

                    /**
                     * temporary list to hold packets we take out of the priority queue and put back
                     * later.
                     */
                    List<Packet> deferredOnes = new ArrayList<>();

                    while (!eligibleDepartures.isEmpty()) {
                        Packet candidatePacket = eligibleDepartures.poll();

                        // wf2q eligibility.
                        if (candidatePacket.virtualStartTime <= clock.virtualTime + EPS) {
                            /**
                             * if we haven't selected any packet (`selectedPacket`) yet OR this
                             * candidatePacket has an earlier finish time.
                             */
                            if (selectedPacket == null ||
                                    candidatePacket.virtualFinishTime < selectedPacket.virtualFinishTime ||
                                    (Math.abs(
                                            candidatePacket.virtualFinishTime - selectedPacket.virtualFinishTime) < EPS
                                            &&
                                            candidatePacket.index < selectedPacket.index)) {

                                // if a previous packet was selected, defer it
                                if (selectedPacket != null) {
                                    deferredOnes.add(selectedPacket);
                                }

                                // our new selected packet
                                selectedPacket = candidatePacket;
                            } else {
                                // keeping candidatePacket aside for reinsertion
                                deferredOnes.add(candidatePacket);
                            }
                        } else {
                            // not yet eligible, deferring it ;)
                            deferredOnes.add(candidatePacket);
                        }
                    }

                    // reinserting not selected packets back to eligibleDepartures
                    eligibleDepartures.addAll(deferredOnes);

                    // if we found a valid eligible packet, we know what to do ;)
                    if (selectedPacket != null) {
                        // start service of course
                        packetInProgress = selectedPacket;
                        packetInProgress.serviceVirtualStartTime = clock.virtualTime;
                    }
                    // if `selectedPacket` remains null, we just do... nothing :D
                } else { // regular PGPS
            ...
            if (packetInProgress != null) {
                ...
                if (nextArrivalTime < clock.realTime + timeToFinish - EPS) {
                    // an arrival occurs before finish: advance to arrival and process
                    advanceTo(nextArrivalTime, totalBackloggedWeight);
                    // loop at top will handle the arrival (we continue without completing)
                    continue;
                } else {
                    // finish before next arrival
                    advanceTo(clock.realTime + timeToFinish, totalBackloggedWeight);
