...
int main(int argc, char * argv[])
{
	...
	string keyInput = argv[3];
	if(keyInput.length() != DES::DEFAULT_KEYLENGTH)
	{
		cout << "Key must be 8 characters long !!!" << endl;
		exit(1);
	}
	memcpy(key, keyInput.data(), DES::DEFAULT_KEYLENGTH);
	byte iv[DES::BLOCKSIZE] = {0};  // 8 byte iv (all zeros for just for test's sake)
